...
export interface FilterBarProps {
  ...
  activePreset: "all" | "high-value";
  onPresetChange: (preset: "all" | "high-value") => void;
}
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg bg-surface-secondary p-4">
      <div className="flex gap-4 border-b border-border pb-2 mb-1">
        <button
          type="button"
          onClick={() => props.onPresetChange("all")}
          className={`text-sm pb-1 transition-colors ${
            props.activePreset === "all"
        ...
        <button
          type="button"
          onClick={() => props.onPresetChange("high-value")}
          className={`text-sm pb-1 transition-colors ${
            props.activePreset === "high-value"
              ? "text-accent-primary border-b-2 border-accent-primary font-medium"
              : "text-text-muted hover:text-text-secondary"
          }`}
        >
      ...
      {props.activePreset === "all" && (
        <>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
            <div>
              <label className="block text-xs text-text-muted">Min score</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-xs text-text-muted">Max score</label>
              <input
                type="number"
                className="mt-1 w-full rounded border border-border bg-surface-primary px-2 py-1 text-sm text-text-primary focus:border-accent-primary focus:outline-none"
            ...
            <div>
              <label className="block text-xs text-text-muted">ZIP</label>
              <input
                type="text"
                className="mt-1 w-full rounded border border-border bg-surface-primary px-2 py-1 text-sm text-text-primary focus:border-accent-primary focus:outline-none"
                value={local.zipCode ?? ""}
            ...
            <div>
              <label className="block text-xs text-text-muted">State</label>
              <input
                type="text"
                className="mt-1 w-full rounded border border-border bg-surface-primary px-2 py-1 text-sm text-text-primary focus:border-accent-primary focus:outline-none"
                value={local.state ?? ""}
