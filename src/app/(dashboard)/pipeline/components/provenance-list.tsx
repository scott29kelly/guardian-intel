  ...
  return (
    <div className="space-y-3">
      {Object.entries(byType).map(([type, recs]) => (
        <div key={type} className="rounded-lg bg-surface-secondary p-3">
          ...
          <ul className="divide-y divide-border/30 text-xs text-text-secondary">
            ...
            {recs.length > 10 && (
              <li className="py-1 text-xs italic text-text-muted">+{recs.length - 10} more</li>
            )}
          </ul>
        </div>
      ))}
